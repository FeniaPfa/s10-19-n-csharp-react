...
const NavUtils = () => {
  ...
  const likeProducts = useSelector(
    (state) => state?.productsLikes?.productLikes
  );
    ...
    >
      ...
      {cantCarProduct > 0 && (
        ...
        >
          ...
          <span className="font-title font-normal  text-[1.1rem] lg:hidden capitalize">
            Carrito
          </span>
      ...
      {logged ? (
        ...
        >
          ...
          <section
            className={`subMenuLogin absolute rounded-md w-[100%] lg:w-[150px] lg:h-[150px] bg-marronCustom text-white flex flex-col gap-2 justify-center items-start p-1 transition-all duration-150 ease-linear ${
              activeloginMenu
                ? ' right-0 top-[46px] lg:top-[75px] lg:right-[-19px] opacity-1 z-10'
                : 'top-0 right-0 lg:right-[-20px] opacity-0 -z-10'
            }`}
          >
            <a
              onClick={() => {
      ...
      >
        ...
        <span className='font-title font-normal  text-[1.1rem] lg:hidden capitalize'>Perfil</span>
      </Link> */}
    </section>
  );
};

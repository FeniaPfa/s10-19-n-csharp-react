...
function Register() {
  ...
  const onSubmit = async (data) => {
    ...
    const res = await axios.post(
      'https://green-eats.onrender.com/users/registration',
      data
    );
  ...
  const styledInput =
    'form-control w-4/5 bg-whiteBackgraundForm2 mx-auto  px-2 py-1.5 text-gray-900 rounded border border-solid border-gray-300 focus:border-gray-200 fouces:outline-none';
  return (
      ...
      <div className=" w-4/5 h-3/5 flex">
        <div className="bg-whiteBackgraundForm2 w-3/6 h-full flex justify-center flex-col ">
          <div className="flex items-start w-4/5  mx-auto">
            <h1 className="text-5x1 sm:text-7x1 text-black text-4xl font-bold px-1 py-3">
              Register
            </h1>
          </div>
          <div>
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="flex flex-col w-full  m-0 items-start p-auto">
                ...
                {errors.email?.type === 'required' && (
                  <div className="flex items-start w-4/5 px-1 mx-auto mt-1.5">
                    <p className={styledErros}>Este campo es obligatorio</p>
                  </div>
                )}
                {errors.email?.type === 'pattern' && (
                  <div className="flex items-start w-4/5 px-1 mx-auto mt-1.5">
                    <p className={styledErros}>Ingresa un email valido</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col">
                ...
                {errors.password?.type === 'minLength' && (
                  <div className="flex items-start w-4/5 px-1 mx-auto mt-1.5">
                    {' '}
                    <p className={styledErros}>Minimo 8 caracteres</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex mt-2 items-start mx-auto w-4/5">
                ...
                <label className="text-sm ms-2  text-gray-600">
                  {' '}
                  Registrar como
                  <a className="text-primary">Restaurante.</a>
                </label>
              </div>
              ...
              <div className="flex flex-col mt-3 items-center">
                <button
                  type="Submit"
                  className="w-4/5 bg-primary text-white mx-auto  px-2 py-2 rounded border border-solid border-gray-300  fouces:outline-none"
                  value="Register0"
                >
                  Registrar
                </button>
                <span className="text-blackCustom mt-3">{alert}</span>
              </div>
            </form>
          </div>
        </div>

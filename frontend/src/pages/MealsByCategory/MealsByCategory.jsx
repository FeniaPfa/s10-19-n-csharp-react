    ...
    <section className="w-full flex flex-col justify-center items-center my-[10%]">
      ...
      <div className="w-3/4 mb-8 flex flex-col gap-2">
        ...
        <p className="font-parrafo font-normal text-sm lg:text-base text-marronCustom">
          Nuestra selección de platos te brinda una experiencia culinaria única,
          con opciones saludables y deliciosas para cada etapa de tu comida.
          Desde entradas frescas hasta postres indulgentes, estamos
          comprometidos con tu bienestar y satisfacción gastronómica.
        </p>
      </div>
      <div className="flex gap-8 flex-wrap w-3/4">
        {!isLoading &&
          data.map((elem) => <CardMenu key={elem.id} object={elem} />)}
        {data.length === 0 && (
          <p className="text-xl font-bold text-black">
            No se han encontrado platillos
          </p>
        )}
      </div>
    </section>
